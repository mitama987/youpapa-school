...
実運用中の X 自動投稿ツール **XToolsPro3** と **XToolsPro4** に組み込まれていた生成プロンプトを、
そのままスキルとして切り出しました。
...
| | モードA 深掘り型 | モードB 量産型 |
|---|---|---|
| 由来 | XToolsPro4 post-generator | XToolsPro3 AIモード |
| ハッシュタグ | 付けない | 2〜3個必須 |
...
- XToolsPro への一括投入に対応：「XTP3の予約用に」でタブ区切り（投稿日時＋投稿内容）、「XTP4用に」で空行区切りテキストを出力し、各ツールの貼り付けインポートにそのまま取り込めます
