...
| スキル | できること | Python |
|---|---|---|
| `/buzzblog` | メモから記事タイトル・本文を生成（画像生成は任意） | **不要**（文章のみ。画像は任意でPython） |
| `/note-edit` | 生成した記事を note.com に書式付きで投稿（下書き保存） | **不要**（ブラウザ操作） |
| `/note-preview` | 投稿前に note 風HTMLでローカル確認 | 必要（任意） |
...
どちらの方法でも、3つのスキルが入ります。`/` を押すと `/buzzblog` などが候補に出ます。
