...
## インストール（Claude Code で3コマンド）

Claude Code を開き、次を順に貼り付けるだけです（Git も cmd も不要）。
...
これで3つのスキルが入ります。`/` を押すと `/note-shukyaku:buzzblog` などが候補に出ます。
（反映されない場合は `/reload-plugins`、それでも出なければ Claude Code を開き直してください）

### うまくいかないとき
- `/plugin marketplace list` で `youpapa-tools` が入っているか確認。
- ブランチ指定が要る場合：`/plugin marketplace add mitama987/youpapa-school@main`
