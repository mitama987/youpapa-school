...
AskUserQuestion ツールでタイトルを選択してもらう。

- **question 本文に Step 2 の10案すべてを番号付き（1.〜10.）で含める（必須）**
  - 10案は question 本文に含めない限りユーザーの画面には一切表示されない（thinking 内で生成した候補は非表示。ツール呼び出し前の地の文テキストも折りたたまれて見えないことがある）
  - 「上の10案」「先ほどの候補」など、画面に表示されていない前提の参照はしない
- 選択肢は代表的なタイトル2〜4件（label は「N. 短縮タイトル」、description にタイトル全文）+ 「Other」（別の番号指定・カスタムタイトル入力）
...
- ver 0.4 / 2026-08-05 / Step 3 修正: タイトル10案が画面に表示されない問題の対策。AskUserQuestion の question 本文に10案全部を番号付きで含めることを必須化（thinking 内で生成した候補は非表示のため「上の10案」参照が空振りしていた）
